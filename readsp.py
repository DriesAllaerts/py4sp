...
def ns_setup(filename='NS.setup',Lref=1.0,Uref=1.0):
    ...
    ns_stp['thermo']      = input[3][0] in ['.TRUE.','.True.','.true.']
    ns_stp['ekman']       = input[5][0] in ['.TRUE.','.True.','.true.']
    ns_stp['Nx2']         = int(input[7][0])
    ns_stp['Ny']          = int(input[7][1])
    ns_stp['Lx']          = float(input[9][0].replace('d','e'))*Lref
    ns_stp['Ly']          = float(input[9][1].replace('d','e'))*Lref
    ns_stp['force_mode']  = int(input[13][0])
    ...
    ns_stp['force_param'] = float(input[13][1].replace('d','e'))*scale
    ns_stp['tstart']      = float(input[15][0].replace('d','e'))*tref
    ns_stp['tstop']       = float(input[17][0].replace('d','e'))*tref
    ns_stp['dt1']         = float(input[19][0].replace('d','e'))*tref
    ns_stp['dt2']         = float(input[21][0].replace('d','e'))*tref
    ns_stp['zmeshf']      = input[25][0]
    ns_stp['z0']          = float(input[31][2].replace('d','e'))*Lref
    ns_stp['model_type']  = int(input[33][0])
...
def en_setup(filename='therm.setup',Lref=1.0,Uref=1.0):
    ...
    if not os.path.exists(filename):
        print('Error: '+filename+' does not exist, en setup file not found')
        return 1
    ...
    en_stp = {}
    en_stp['gravity']   = float(input[1][0].replace('d','e'))*Uref**2/Lref
    return en_stp
...
def ek_setup(filename='ekman.setup',Lref=1.0,Uref=1.0):
    ...
    ek_stp = {}
    Ro = float(input[1][0].replace('d','e'))
